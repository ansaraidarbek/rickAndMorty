const createLeft = (element, leftWindow, notify) =>{
    ...
    leftWindow.innerHTML = `
        ...
        <div class='window_favourite'>
            ${inFavourite(element.id)}
        </div>
    `;

    const button = document.querySelector('.window_toggleFav');

    button.addEventListener('click', (e) => {
        e.preventDefault();
        notify(element.id);
        createLeft(element, leftWindow);
    })
}

const createRight = (element, rightWindow) => {
    const createElem = (text1, text2) => {
    ...
    const stringList = ['gender', 'species', 'type', 'status']
    const objList = ['location', 'origin', ]
    stringList.forEach(str => {
        if (element[str]) {
            rightWindow.appendChild(createElem(str, element[str]));
        }
    })
    objList.forEach(str => {
        if (element[str]) {
            rightWindow.appendChild(createElem(str, element[str].name));
        }
    })
    
}

export function info (active, id, leftWindow, rightWindow, notify) { 
    const obj = !active ? JSON.parse(localStorage.getItem('localCache')) : JSON.parse(localStorage.getItem('favourites'));
    if (obj && obj?.storage && id) {
        obj.storage.results.forEach(el => {
            if (el.id === +id) {
                createLeft(el, leftWindow, notify);
                createRight(el, rightWindow);
                return;
            }
        })
    }
